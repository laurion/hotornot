...
router.post('/login', function(req, res) {//todo for 
	...
	var obj = new Parse.Object('popovici');
   var query = new Parse.Query('popovici');
      ...
      if(objAgain != null){
    	  var score = objAgain.get('score');
      	  res.end(JSON.stringify(score));
  	   	  ...
  	   	  var newobj = new Parse.Object('popovici');
...
function loadRootPage(req,res,next) {
   ...
   var obj = new Parse.Object('popovici');
   var query = new Parse.Query('popovici');
   var queryCurrentUser = new Parse.Query('popovici');
   ...
   query.find().then(function(users) {// query to fetch top scorers
	  ...
	  var data = {
        ...
        method: 'POST',
        json: true,
        body: {leaderboard_list: users, cluster : "popovici", password:"4loc4"}
     };
 ...
 var gender ;
  ...
  /* query.find().then(function(users) {// query to fetch top scorers
     ...
     var data = {
        ...
        method: 'POST',
        json: true,
        body: {fbId: sessid, cluster : "popovici", password:"4loc4"}
     };
    return Promise.resolve(requestLib(data)).then(function(redis ){
		...
		if(redis.body != null) {
			...
			if(gender == null){
		  	 ...
		  	 var obj = new Parse.Object('popovici');
  		     var queryUser = new Parse.Query('popovici');
...
router.post("/interested_in", function(req, res){
	...
	var sessid = req.session.id;
     ...
     var data = {
        ...
        method: 'POST',
        json: true,
        body: {sessionId: sessid, cluster : "popovici", password:"4loc4", gender: req.body.interested_in}
     };
   requestLib(data);
