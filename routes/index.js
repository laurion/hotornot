      ...
      if(objAgain != null){
    	  var score = objAgain.get('score');
      	  res.end(JSON.stringify(score));
  	   	  ...
  	   	  var newobj = new Parse.Object('Racovita');
...
function loadRootPage(req,res,next) {
   ...
   var obj = new Parse.Object('Racovita');
   var query = new Parse.Query('Racovita');
   var queryCurrentUser = new Parse.Query('Racovita');
   ...
   query.find().then(function(users) {// query to fetch top scorers
	  ...
	  var data = {
        ...
        method: 'POST',
        json: true,
        body: {leaderboard_list: users, cluster : "racovita", password:"4loc4"}
     };
...
router.get('/voted/:voteValue/:fbId/:nrOfVotes/:score', function(req, res, next) {
 ...
 var gender ;
  //register vote to parse for last generated person for this user (only if exists)
   ...
   var skip = 0;
   var obj = new Parse.Object('Racovita');
   var query = new Parse.Query('RacovitaRacovita');
   var hasUser = 0;
   var hasTop = false;
  ...
  /* query.find().then(function(users) {// query to fetch top scorers
     ...
     var data = {
        ...
        method: 'POST',
        json: true,
        body: {fbId: sessid, cluster : "racovita", password:"4loc4"}
     };
    return Promise.resolve(requestLib(data)).then(function(redis ){
		...
		if(redis.body != null) {
			...
			if(gender == null){
		  	 ...
		  	 var obj = new Parse.Object('Racovita');
  		     var queryUser = new Parse.Query('Racovita');
...
router.post("/interested_in", function(req, res){
	...
	var sessid = req.session.id;
     ...
     var data = {
        ...
        method: 'POST',
        json: true,
        body: {sessionId: sessid, cluster : "racovita", password:"4loc4", gender: req.body.interested_in}
     };
   requestLib(data);
